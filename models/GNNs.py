...
from torch.nn import GRUCell, Linear, Parameter, ModuleList
...
class BasicGNN(torch.nn.Module):
    ...
    def forward(
        ...
        emb = self.jk(xs) if hasattr(self, 'jk') else x
        emb = global_mean_pool(emb, batch)
        x = self.lin(emb) if hasattr(self, 'lin') else emb
...
class GINE(BasicGNN):
    ...
    def init_conv(self, in_channels: int, out_channels: int,
                  edge_dim: Optional[int] = None, **kwargs) -> MessagePassing:
        mlp = MLP(
            [in_channels, out_channels, out_channels],
            act=self.act,
            act_first=self.act_first,
