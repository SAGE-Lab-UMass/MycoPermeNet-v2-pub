...
enamine_nst_df = pd.read_csv('./results/archived/chemprop_permeability_enamine_nst_checkpoint/chemprop_test_performance.csv')
enamine_fnst_df = pd.read_csv('./results/archived/chemprop_permeability_enamine_fusion_nst_checkpoint/chemprop_test_performance.csv')

# ================================
# Extract RMSE columns
# ================================
baseline = baseline_nst_df["Iter0 RMSE"].values
mlsmr_nst = baseline_nst_df["Iter2 RMSE"].values
mlsmr_fnst = baseline_fnst_df["Iter2 RMSE"].values

enamine_nst = enamine_nst_df["Iter3 RMSE"].values
enamine_fnst = enamine_fnst_df["Iter1 RMSE"].values
...
# ================================
comparisons = [
    ("MLSMR Baseline vs. Baseline + NST", baseline, mlsmr_nst),
    ("MLSMR Baseline vs. Baseline Fusion+NST", baseline, mlsmr_fnst),
    ("Enamine Baseline vs. Baseline + NST", baseline, enamine_nst),
    ("Enamine Baseline vs. Baseline Fusion+NST", baseline, enamine_fnst),
]
