...
@torch.no_grad()
def get_representations(model, loader, NST=False, scaler=None, fusion=False, cross_attn=False):
    model.eval()
    smiles_list = []
    embeddings = []
    y_list = []
    ...
    # if smile:
    #     df_embed = pd.DataFrame(X, columns=[f'ft_{i}' for i in range(X.shape[1])])
...
for i, (torch_seed, data_seed) in enumerate(seed_combinations):
    ...
    for epoch in tqdm(range(1, 51)):
        if use_MINE:
            loss, train_mi_loss = train(model, train_loader, optimizer, scheduler, mine, combined_weight)
            val_mae, val_mi_loss = test(model, val_loader, mine, combined_weight)
            test_mae, _ = test(model, test_loader, mine, combined_weight)
        else:
    ...
    train_loader, val_loader, test_loader = get_emb_dataloaders_from_molnet(
    ...
    if cross_attn:
        ...
        X_test, y_test = get_representations(model, test_loader, fusion=fusion)
    ...
    if not NST:
        ...
        for iter_i in range(1, nst_iterations + 1):
            ...
            if i == 0:
                ...
                if fusion:
                    unlabeled_dataset = get_dataset_from_mlsmr(smiles_chunk, descriptor_names=descriptor_names)
                else:
                    unlabeled_dataset = get_dataset_from_mlsmr(smiles_chunk)
