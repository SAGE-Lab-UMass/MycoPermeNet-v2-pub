"Save the best MycoPermeNet-v2 model and evaluate its performance."
...
for i, (torch_seed, data_seed) in enumerate(seed_combinations):
    ...
    for epoch in tqdm(range(1, 51)):
        ...
        if val_mae < best_val:
            best_val = val_mae
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(save_dir, 'best_GNN.pt'))
        else:
            patience_counter += 1
    ...
    if fusion:
        ...
        X_train.to_csv("./model/train_emb.csv", index=False)
        X_val.to_csv("./model/val_emb.csv", index=False)
        X_test.to_csv("./model/test_emb.csv", index=False)
