...
if fusion and use_MINE:
    ...
    save_dir = f'./results/{GNN}_permeability_easy_fusion_nst_checkpoint'
elif fusion:
    ...
    save_dir = f'./results/{GNN}_permeability_easy_checkpoint'
os.makedirs(save_dir, exist_ok=True)

whole_df = pd.read_csv('./data/siegrist_clean_mtb1600.csv')
train_df = pd.read_csv('./data/train_easy_split.csv')
test_df = pd.read_csv('./data/test_easy_split.csv')
...
for i, (torch_seed, data_seed) in enumerate(seed_combinations):
    ...
    if i == 0:
        whole_dataset = get_dataset(whole_df)
        train_val_dataset = get_dataset(train_df)
        test_dataset = get_dataset(test_df)
        torch.save(whole_dataset, os.path.join(save_dir, 'whole_dataset.pt'))
        ...
        whole_dataset = torch.load(os.path.join(save_dir, 'whole_dataset.pt'))
    ...
    train_loader, val_loader, test_loader, target_scaler = get_dataloaders(
        train_val_dataset=whole_dataset,
